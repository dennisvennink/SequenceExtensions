  ...
  class TakeLastTests: XCTestCase {
    ...
    func testShouldReturnValidResults () {
      self.validate(Array([1, 2, 3].lazy.take(last: 0)), against: [Int]())
      self.validate(Array([1, 2, 3].lazy.take(last: 1)), against: [3])
      self.validate(Array([1, 2, 3].lazy.take(last: 2)), against: [2, 3])
      self.validate(Array([1, 2, 3].lazy.take(last: 3)), against: [1, 2, 3])
      self.validate(Array([1, 2, 3].lazy.take(last: 4)), against: [1, 2, 3])
    }

    func testResultsShouldBeLazyEvaluated () {
      iterationCount = 0

      _ = [1, 2, 3].lazy.take(last: 3)
